...
PROJECTNAME:=gotraceroute
BUILDPATH:=${PROJECTPATH}/cmd
...
build: dep ## Build executable.
	mkdir -p ./build
	CGO_ENABLED=0 GOOS=linux GOARCH=${GOARCH} go build ${LDFLAGS} -o build/${PROJECTNAME} ./cmd
...
test: dep ## Run tests
	go test -c
	sudo setcap cap_net_raw+ep ./${PROJECTNAME}.test
	go test -exec ./${PROJECTNAME}.test -race -p 1 -timeout 300s -coverprofile=.test_coverage.txt ./... && \
